...
function register(
  ...
  let currentController = isInstance
  ...
  Object.getOwnPropertyNames(currentController).forEach((key) => {
    ...
    if (!excludeKeys.includes(key) && value instanceof Function) {
      ...
      let controllerPath = Reflect.get(
        isInstance ? currentController.constructor : currentController,
        "path",
      );
      ...
      routerFunc.call(router, path, async (ctx: ParameterizedContext) => {
        ...
        if (
          beforeMethodCallHookFn &&
          typeof beforeMethodCallHookFn === "function"
        ) {
        ...
        try {
          if (func.rateLimitInstance && func.rateLimitConsumeFn) {
            await func.rateLimitConsumeFn.call(
              null,
              func.rateLimitInstance,
              ctx,
            );
          }
        } catch (error) {
...
export function scanControllerAndRegister(
  router: Router,
  scanControllerOpts: ScanControllerOpts,
) {
